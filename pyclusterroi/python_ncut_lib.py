...
def ncut(w, num_eigen_values):
    """
    (eigen_val, eigen_vec) = ncut( W, num_eigen_values ):
    ...
    nbEigenValues eigenvectors are calculated. The number of eigenvectors corresponds
    to the maximum number of classes (K) that will be produced by the
    clustering algorithm.

    :param w: symmetric #feature x #feature sparse matrix representing the
    ...
    :param num_eigen_values: number of eigenvectors that should be calculated, this
    ...
    :return: (eigen_val, eigen_vec)
        eigen_val: (output) eigenvalues from the eigen decomposition of the
                   LaPlacian of W
        eigen_vec: (output) eigenvectors from the eigen decomposition of the
                   LaPlacian of W
    """
    ...
    d_inv_sqrt = spdiags((1.0 / sqrt(d + eps)), [0], m, m, "csc")
    p = d_inv_sqrt * (w * d_inv_sqrt)

    # perform the eigen decomposition
    (eigen_val, eigen_vec) = eigsh(p, num_eigen_values,
    ...
    eigen_vec = d_inv_sqrt * matrix(eigen_vec)
